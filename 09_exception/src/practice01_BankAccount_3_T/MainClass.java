...
public class MainClass {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      // catch 블록의 RuntimeException e 로 전달되는 인자
      // 1. catch (DepositException e) {throw new RuntimeException(money + "원 입금불가", e);}
      // 2. catch (WithdrawalException e) {throw new RuntimeException(money + "원 입금불가", e);}
      // 3. catch (WithdrawalException e) {throw new RuntimeException("잔액부족", e);} 
      
      String message = e.getMessage();
      System.out.println(message);                        // 예외메시지 확인
      
      BankException cause = (BankException) e.getCause(); // 원인객체로부터 얻어낼 수 있는 것
