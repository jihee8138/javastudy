...
public class MainClass {
  ...
  /*
   * (파일들의) 경로 작성 방법
   * 1. 경로 구분자 (separator) (운영체제 따라감)
   *    1) 윈도우      : 역슬래시(\\) 
   *    2) 리눅스, Mac : 슬래시(/)
   *    3) File.separator(static 멤버) 필드를 사용하면 플랫폼에 따른 경로 구분자가 자동 삽입된다.
   * 2. 디렉터리
  ...
  public static void method1() {
    ...
    // if(dir.exists() == false) 가독성을 높이기 위해 이렇게 쓰기도 함
      dir.mkdirs();
    } else {             // 디렉터리가 있으면
  ...
  public static void method2() {
    ...
    if(file.exists()) {  // 파일이 있으면
      ...
      try {
        file.createNewFile();  // IOException 처라거 반드시 필요하다. (Checked Exception)
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
  ...
  public static void method3() {
     ...
     * 5. lastModified() : 최종수정일을 long 타입으로 반환 (Timestamp 값) 반환
...
//    practice2(new File("\2024\01\22"))
  ...
  public static void practice2(File dir) {
    ...
    // 파라미터 File dir 객체의 상취 디렉터리가 없으면 종료
